...
* a **graded** canary (label + Gaussian noise at several SNRs) must degrade gracefully, so we
  know the framework's sensitivity floor — real leaks are subtle, not perfect.
"""
...
def test_graded_canary_degrades_gracefully() -> None:
    # label + Gaussian noise at increasing strength -> weaker leak -> AUC drops toward 0.5.
    rng = np.random.default_rng(7)
    frame, y = _base(2000, seed=1)
    aucs = []
    for noise_sd in (0.0, 0.5, 1.0, 2.0, 4.0):
        leak = y + rng.standard_normal(len(y)) * noise_sd
    ...
    assert aucs[-1] < 0.65  # heavily-corrupted leak approaches chance
